...
class UserServicesMock: UserServicesProtocol {
    ...
    func checkUserLogged(callBack: @escaping (Result<Hunter, Error>) -> Void) {
        if let userLogged = fakeData?.userSign, fakeData?.resonseError == nil {
            let hunter = Hunter(displayName: userLogged.displayName, user: userLogged)
            callBack(.success(hunter))
        } else {
            callBack(.failure(fakeData?.resonseError ?? ServicesError.signIn))
        }
    }
